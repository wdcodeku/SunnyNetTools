...
- Windows amd64：`SunnyNetTools-v1.0.2-windows-amd64.zip`
- Linux amd64：`SunnyNetTools-v1.0.2-linux-amd64.tar.gz`
- macOS universal（Intel + Apple Silicon）：`SunnyNetTools-v1.0.2-macos-universal.zip`

推送到 `main` 后会更新 GitHub Releases 中的 `continuous` 预发布版本；推送形如
`v1.0.2` 的标签后会创建对应的正式 Release。Pull Request 只执行三端构建验证，不发布。
